...
- `材料收件箱`：把杂乱的 CV 全文、职位描述、HR / recruiter 聊天截图、个人项目笔记、头像、PDF 或其他文件一次粘贴或拖入。PDF 会先在本机提取文字并渲染页面预览；视觉模型按语义提取、分类。本地模式不会假装识图，未读懂的图片 / 扫描 PDF 不能直接入库。
- `素材银行`：真正入库的是确认后的结构化 CV、职位描述和个人资料；原始粘贴、HTML 与附件只作为本机来源凭证。可以反复选择不同目标职位，生成多个互不覆盖的独立 LaTeX 项目，并自动加入“我的 CV”。
...
材料收件箱的分类结果必须先由用户确认才会写入 `.cvstudio-bank/`，客户端和服务端都会阻止没有提取内容的附件占位条目。生成器默认使用 [geekplux/cv_resume](https://github.com/geekplux/cv_resume) 的 MIT 许可蓝色模板方向；CV Studio 的 portable edition 保留归属和许可证，同时移除原模板对 `moderncv` 与特定字体的硬依赖，已使用内置 Tectonic 离线编译验证。详细数据流见 [`docs/material-intake.md`](docs/material-intake.md)。
